...
class SearchView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        album_query = Q(title__icontains=query)
        artist_query = Q(a_name__icontains=query)
        song_query = Q(title__icontains=query) | Q(s_artist__a_name__icontains=query) | Q(s_album__title__icontains=query)
        
        search_results = [
            {
                'type': 'album',
                'data': AlbumSerializer(album).data,
                'score': album.title.name().count(query.lower()),
            }
